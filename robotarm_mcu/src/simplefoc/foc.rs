...
use crate::{
    ...
    simplefoc::{
        ...
        pid::PIDController,
        types::{_2PI, NOT_SET, PhaseVoltages, SensorDirection, TorqueControlType},
    },
};
...
impl<'a, ENCODER: EncoderSensor, CURRENT: CurrentSensor> SimpleFOC<'a, ENCODER, CURRENT> {
    ...
    pub fn set_target_torque(&mut self, torque: f32) {
        ...
        // self.motor.target_current = torque * self.sensor_direction.multiplier();
    }

    pub fn set_target_velocity(&mut self, velocity: f32) {
        self.motor.target_shaft_velocity = velocity;
        // self.motor.target_shaft_velocity = velocity * self.sensor_direction.multiplier();
    }
...
impl<'a, ENCODER: EncoderSensor, CURRENT: CurrentSensor> SimpleFOC<'a, ENCODER, CURRENT> {
    /// skip LPF for use with state observer
    // #[cfg(feature = "nope")]
    pub(super) fn get_shaft_velocity(&self, t_us: u64) -> f32 {
        // self.sensor_direction.multiplier() * self.encoder.get_velocity()
        // self.sensor_direction.multiplier() * self.state_observer.get_angle_vel().1
        // self.state_observer.get_angle_vel().1
        // unimplemented!()
    ...
    pub(super) fn get_shaft_angle(&self) -> f32 {
        // let angle = self.encoder.get_angle();
        // let angle = self.state_observer.get_angle_vel().0;
        // let angle =
        //     // self.sensor_direction.multiplier() * self.lpf_angle.filter(angle) - self.sensor_offset;
        ...
        // self.sensor_direction.multiplier() * self.state_observer.get_state_angle()
    }
    ...
    pub(super) fn get_mechanical_angle(&self) -> f32 {
        ...
        // - self.sensor_offset;
        // Self::normalize_angle(angle)
        Self::normalize_angle(self.get_shaft_angle())
    }

    pub(super) fn get_electrical_angle(&self) -> f32 {
        // let shaft_angle = self.get_mechanical_angle();
        // // let shaft_angle = self.encoder.get_mechanical_angle();
        // // let angle = self.sensor_direction.multiplier() * self.motor.pole_pairs as f32 * shaft_angle
        // let angle = self.motor.pole_pairs as f32 * shaft_angle - self.zero_electric_angle;
    ...
    fn get_encoder_electrical_angle(&self) -> f32 {
        let shaft_angle = self.encoder.get_mechanical_angle();
        let angle = self.sensor_direction.multiplier() * self.motor.pole_pairs as f32 * shaft_angle
            - self.zero_electric_angle;
...
impl<'a, ENCODER: EncoderSensor, CURRENT: CurrentSensor> SimpleFOC<'a, ENCODER, CURRENT> {
    ...
    pub(super) fn angle_openloop(&mut self, target: f32, now_us: u64) -> f32 {
        ...
        if (target - self.openloop_shaft_angle).abs() > (max_vel * t_us).abs() {
            self.openloop_shaft_angle +=
                (target - self.openloop_shaft_angle).signum() * (max_vel * t_us).abs();

            // debug!(
            //     "setting phase voltage: {}",
            //     self.openloop_shaft_angle * self.motor.pole_pairs as f32
            // );

            self.set_phase_voltage(
                self.motor.limit_voltage,
                0.,
                self.openloop_shaft_angle * self.motor.pole_pairs as f32,
            );
        } else {
            self.openloop_shaft_angle = target;
            self.set_phase_voltage(
                0.,
                0.,
                self.openloop_shaft_angle * self.motor.pole_pairs as f32,
